...
OUT_DATA_SIG=wmt14_en_de-bert-or-bart-or-electra
...
ELECTRA_DIR=$ROOT/pretrain_models/electra-base-discriminator
...
#ln -sf $INPUT_CON.bart $INPUT.mult.electra.$SOURCE
#
## # link mapping files
#ln -sf $INPUT_CON.data_dict.electra-base-discriminator.map $INPUT.mult.electra.map.$SOURCE
...
python3 $ROOT/preprocess.py --source-lang en --target-lang de \
--trainpref $INPUT \
...
--electra-model-name $ELECTRA_DIR
