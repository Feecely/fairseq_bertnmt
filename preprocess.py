...
def binarize(args, filename, vocab, output_prefix, lang, offset, end, avoid_tokenize=False, append_eos=True):
    ds = indexed_dataset.make_builder(
        dataset_dest_file(args, output_prefix, lang, "bin"),
        impl=args.dataset_impl,
    ...
    res = Binarizer.binarize(
        filename, vocab, consumer, append_eos=append_eos, offset=offset, end=end, avoid_tokenize=avoid_tokenize
    )
    ds.finalize(dataset_dest_file(args, output_prefix, lang, "idx"))
    return res
