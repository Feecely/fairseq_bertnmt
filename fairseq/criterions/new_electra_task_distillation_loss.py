...
@dataclass
class NewElectraTaskDistillationLossCriterionConfig(FairseqDataclass):
...
@register_criterion(
    "new_electra_task_distillation_loss", dataclass=NewElectraTaskDistillationLossCriterionConfig
)
class NewElectraTaskDistillationLossCriterion(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def forward(self, model, sample, reduce=True):
        ...
        electra_task_out, electra_encoder_out = ret['electra_task_out'], ret['electra_encoder_out']
        electra_task_loss = ret['electra_task_loss']
        if self.kd_feature == 'logits':
            loss_kd = self.MSE_loss(electra_encoder_out, electra_task_out)
        elif self.kd_feature == 'logprobs':
            loss_kd = self.MSE_loss(self.log_sm(electra_encoder_out), self.log_sm(electra_task_out))
        else:
        ...
        logging_output = {
            ...
            "electra_task_loss": electra_task_loss.data,
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        electra_task_loss_sum = sum(log.get("electra_task_loss", 0) for log in logging_outputs)
